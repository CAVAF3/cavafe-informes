...
export default function App() {
  ...
  const docsReq = tipo === "Recuperación de Certificados" 
    ? [
        "Factura CFDI (parte 1)", 
        "Factura CFDI (parte 2)", 
        "Correo de contestación emisor", 
        "Captura validación comprobante fiscal", 
        "Captura certificados SAT (parte 1)", 
        "Captura certificados SAT (parte 2)", 
        "Captura certificado individual",
        "Captura validación RFC", 
        "Captura validación RFC y código postal"
      ] 
  ...
  const doAnalysis = async () => {
    ...
    try {
      ...
      setAnalisisIA(analyzeData.analisisIA);
      ...
      const wordResponse = await fetch(
        esRecuperacionCertificados ? '/api/generar-word-certificados' : '/api/generar-word',
        {
          ...
          body: JSON.stringify({
            ...
            analisisIA: analyzeData.analisisIA,
