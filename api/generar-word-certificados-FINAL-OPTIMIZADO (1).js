    ...
    if (analisisIA.coordenadas_recortes && facturas.length > 0) {
      ...
      const recortar = async (nombre, c) => {
        try {
          ...
          const metadata = await sharp(facturas[idx]).metadata();
          ...
          if (c.x_percent !== undefined) {
            // Coordenadas en porcentaje
            left = Math.max(0, Math.round((c.x_percent / 100) * imgWidth));
            top = Math.max(0, Math.round((c.y_percent / 100) * imgHeight));
            width = Math.round((c.width_percent / 100) * imgWidth);
            height = Math.round((c.height_percent / 100) * imgHeight);
          } else {
            // Coordenadas en píxeles (compatibilidad con formato antiguo)
            left = Math.max(0, c.x);
            top = Math.max(0, c.y);
            width = c.width;
            height = c.height;
          ...
          console.error(`Error recortando ${nombre}:`, e.message);
          return null;
        }
      };
