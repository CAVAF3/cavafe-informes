...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (camposFaltantes.length > 0) {
      console.error("[ERROR] Campos faltantes en analisisIA:", camposFaltantes);
    ...
    console.log(`[DIR] Buscando recursos en: ${assetsPath}`);
    ...
    try {
      ...
      console.log(`[FILES] Archivos en assets:`, filesInAssets);
      ...
      console.log("[OK] Recursos cargados correctamente");
      console.log(`   - Logo CAVAFE: ${logoCavafe.length} bytes`);
      console.log(`   - Firma: ${firmaManuel.length} bytes`);

    } catch (assetError) {
      console.error("[ERROR] Error cargando recursos:", assetError.message);
    ...
    for (let i = 0; i < imagenes.length; i++) {
      try {
        ...
        if (!img.data) {
          console.warn(`[WARN] Imagen ${i + 1} no tiene data, saltando...`);
          continue;
        }

        // Filtrar PDFs (no se pueden insertar como imagen)
        if (img.type === 'application/pdf') {
          console.warn(`[WARN] Archivo ${i + 1} es PDF, no se puede insertar como imagen`);
          continue;
        }
        ...
        if (!base64Data || base64Data.trim() === '') {
          console.warn(`[WARN] Imagen ${i + 1} está vacía, saltando...`);
          continue;
        }

        const buffer = Buffer.from(base64Data, 'base64');
        facturaImagenes.push(buffer);
        console.log(`[OK] Imagen ${i + 1} procesada: ${buffer.length} bytes`);
        
      } catch (imgError) {
        console.error(`[ERROR] Error procesando imagen ${i + 1}:`, imgError.message);
      }
    }

    if (facturaImagenes.length === 0) {
      console.error("[ERROR] No se pudo procesar ninguna imagen");
      return res.status(400).json({ 
        error: "No se pudieron procesar las imágenes de la factura" 
      });
    }

    console.log(`[OK] ${facturaImagenes.length} imagen(es) de factura procesada(s)`);

    // GENERAR RECORTES DE LA FACTURA
    console.log("[PROCESS] Generando recortes de la factura...");
    const recortes = {};
    ...
    if (analisisIA.coordenadas_recortes) {
      try {
        ...
        const recortarImagen = async (nombreRecorte, coordenada) => {
          try {
            if (!coordenada || !coordenada.imagen) {
              console.warn(`[WARN] Coordenada ${nombreRecorte} no válida`);
              return null;
            }
            
            const imagenIndex = coordenada.imagen - 1; // Convertir de 1-based a 0-based
            if (imagenIndex >= facturaImagenes.length) {
              console.warn(`[WARN] Imagen ${coordenada.imagen} no existe para ${nombreRecorte}`);
              return null;
            }
            ...
            console.log(`[OK] Recorte ${nombreRecorte} generado: ${recorte.length} bytes`);
            return recorte;
          } catch (error) {
            console.error(`[ERROR] Error recortando ${nombreRecorte}:`, error.message);
            return null;
          }
        };
        ...
        console.log(`[OK] Recortes generados exitosamente`);
      } catch (recorteError) {
        console.error("[ERROR] Error generando recortes:", recorteError.message);
        // Continuamos sin recortes
      }
    } else {
      console.warn("[WARN] No se recibieron coordenadas de recortes");
    }
    ...
    // GENERAR DOCUMENTO
    console.log("[DOC] Generando documento Word...");
    ...
    console.log("[PACK] Empaquetando documento...");
    const buffer = await Packer.toBuffer(doc);
    console.log(`[OK] Documento generado: ${buffer.length} bytes`);
    ...
    console.error("[ERROR] Error generando Word:");
